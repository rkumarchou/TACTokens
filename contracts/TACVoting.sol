...
contract TACVoting {
  ...
  // avoids a Proposal with ID 0 because it is used
  event SpecialProposalAdded(uint ProposalID, uint _incrementPercentage, string description);
  ...
  function newProposal (string _description, bytes _transactionData, uint64 _debatingPeriod)
  onlyowner
  returns (uint _ProposalID) {
    ...
    ProposalAdded(_ProposalID, _description);
  }

  function specialProposal (string _description, bytes _transactionData, uint64 _debatingPeriod, uint8 _incrementPercentage)
  onlyowner
  returns (uint _proposalID) {
    ...
    SpecialProposalAdded(_proposalID, _incrementPercentage,_description);
  }

  function setTokenContracts (address newTokenContractAddr, address newTokenBonusAddr) onlyowner {
    require (newTokenContractAddr != address(0x0));
    // Allow setting only once
    require (tokenControl == address(0x0));
    tokenControl = TACtoken(newTokenContractAddr);

    require (newTokenBonusAddr != address(0x0));
    // Allow setting only once
    require (tokenBonusContract == address(0x0));
    tokenBonusContract = TokenBonus(newTokenBonusAddr);
  }
  ...
  function vote (uint _ProposalID, bool _supportsProposal) {
    ...
    if (_supportsProposal) {
        ...
        p.nay += tokenControl.balanceOf(msg.sender);
  ...
  function minQuorum() internal constant returns (uint _minQuorum) {
    ...
    return tokenControl.totalSupply() / minQuorumDivisor;
  }
  ...
  returns (bool _success) {
    ...
    require (now >= p.votingDeadline  // has the voting deadline arrived?
     ...
     if (quorum >= minQuorum() && p.yea > p.nay) {
       ...
       tokenBonusContract.addIncrement(p.incrementPercentage);
       }
     }

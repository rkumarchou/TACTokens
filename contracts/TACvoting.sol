...
contract TACvoting {
  ...
  tokenIncrement tokenIncrementControl;
  ...
  function specialProposal (
    ...
    function setTokenContracts (address newTokenContractAddr, address newTokenIncrementAddr) onlyowner {
        ...
        require (newTokenIncrementAddr != address(0x0));
        // Allow setting only once
        require (tokenIncrementControl == address(0x0));
        tokenIncrementControl = tokenIncrement(newTokenIncrementAddr);
    }
        ...
        if (now < p.votingDeadline  // has the voting deadline arrived?
            ...
            || p.proposalHash != sha3(_transactionData)
               ...
               if (quorum >= minQuorum() && p.yea > p.nay) {
                   ...
                   tokenIncrementControl.addIncrement(p.incrementPercentage);
                  }
